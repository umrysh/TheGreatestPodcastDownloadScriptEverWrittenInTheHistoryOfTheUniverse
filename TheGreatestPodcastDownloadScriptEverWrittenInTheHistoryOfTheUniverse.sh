...
oldDir="$( ./readlink.sh -f "$( dirname "$0" )" )"
#
#
#### Menus ####
#
...
then
	...
	# MacOS
		command -v mpg321 >/dev/null 2>&1 || { echo >&2 "I require mpg321 but it's not installed. Run 'brew install mpg321'"; exit 1; }
		command -v soundstretch >/dev/null 2>&1 || { echo >&2 "I require soundstretch but it's not installed."; exit 1; }
		command -v lame >/dev/null 2>&1 || { echo >&2 "I require lame but it's not installed. Run 'brew install lame'"; exit 1; }
		command -v id3cp >/dev/null 2>&1 || { echo >&2 "I require id3lib but it's not installed. Run 'brew install id3lib'"; exit 1; }
		command -v watch >/dev/null 2>&1 || { echo >&2 "I require watch but it's not installed. Run 'brew install watch'"; exit 1; }
	fi
else
	...
	then
		...
		# Arch
		command -v mpg123 >/dev/null 2>&1 || { echo >&2 "I require mpg321 but it's not installed. Run 'pacman -S mpg123' as root."; exit 1; }
